...
import { Player, AuctionState, GameSettings, Character } from '../../types/game';
import { DollarSign, Layers, Crown, CheckCircle2, Sparkles, Trash2 } from 'lucide-react';
...
export function PlayerHUD({
  ...
  const handleOpenDiscard = (e: React.MouseEvent, playerId: string, character: Character, count: number) => {
    e.stopPropagation();
  ...
  const handleConfirmDiscard = () => {
    if (discardTarget && onDiscardCharacter) {
      onDiscardCharacter(discardTarget.playerId, discardTarget.character.id);
    }
  ...
  return (
    <>
          ...
          return (
            ...
            >
              <div>
                ...
                {/* Owned Roster Mini-Cards with Discard Click Option */}
                <div className="flex gap-1 overflow-x-auto py-1 min-h-[36px] items-center border-t border-white/5">
                  {player.collection.map((char, idx) => (
                    <div 
                      key={idx} 
                      title={`${char.name} (${char.grade}) — Click to Discard ($0 Refund)`}
                      onClick={(e) => handleOpenDiscard(e, player.id, char, player.collection.length)}
                      className="relative group/mini cursor-pointer"
                    >
                      <CharacterPortrait character={char} size="sm" showBadge={false} className="w-7 h-7 rounded hover:opacity-80 transition-opacity" />
                      <span className="absolute inset-0 bg-red-600/80 text-white rounded flex items-center justify-center opacity-0 group-hover/mini:opacity-100 transition-opacity">
      ...
      {discardTarget && (
        <DiscardConfirmModal
          character={discardTarget.character}
          currentCount={discardTarget.playerCount}
