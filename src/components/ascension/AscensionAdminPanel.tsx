...
export const AscensionAdminPanel: React.FC = () => {
  ...
  useEffect(() => { if (isAdmin && (section === 'players' || section === 'danger')) loadPlayers(playerPage, playerSearch); }, [isAdmin, section, playerPage]);
  ...
  return (
    <div className="flex min-h-[720px] flex-col gap-5 text-slate-100 lg:flex-row">
      ...
      <section className="min-w-0 flex-1 space-y-5">
        ...
        {section === 'danger' && <div className={`${card} border-rose-500/30 p-5`}><div className="mb-4 flex items-center gap-3"><ShieldAlert className="h-6 w-6 text-rose-400" /><div><h3 className="text-xl font-black text-rose-300">Danger Zone</h3><p className="text-xs text-slate-400">Destructive player actions require confirmation and are recorded in the audit log.</p></div></div><div className="mb-4 flex flex-wrap gap-2"><select className={`${input} max-w-xl`} value={selected?.id || ''} onChange={async event => { const player = players.find(candidate => candidate.id === event.target.value); if (player) await openPlayer(player); }}><option value="">Select a player to manage</option>{players.map(player => <option key={player.id} value={player.id}>{player.displayName} (@{player.username})</option>)}</select><button onClick={() => loadPlayers(1, playerSearch)} className="rounded-xl border border-slate-700 px-4 text-xs font-bold text-cyan-300">REFRESH PLAYERS</button></div>{selected && details ? <PlayerDetail details={details} selected={selected} action={action} setAction={setAction} amount={amount} setAmount={setAmount} actionCharacter={actionCharacter} setActionCharacter={setActionCharacter} actionMessage={actionMessage} onApply={applyAction} onClose={() => { setDetails(null); setSelected(null); }} /> : <div className="rounded-xl border border-rose-500/20 bg-rose-950/20 p-6 text-center text-sm text-slate-300">Choose a player above to open moderation controls.</div>}</div>}
